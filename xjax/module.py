...
from xjax.xnn import Linear, Conv, Deconv, ResizeLike, Sigmoid


def Residual(module1, module2, transfer=Sigmoid, resize=ResizeLike):
...
def ResLinear(in_dim, feat_dim, out_dim, transfer=Sigmoid, resize=ResizeLike,
...
def ResConv(in_dim, feat_dim, out_dim, first_kernel, second_kernel,
            ...
            transfer=Sigmoid, resize=ResizeLike, w_init=jinit.normal(1e-6),
...
def Discriminator(level, depth, in_dim, feat_dim, out_dim, kernel=(3,),
                  pool=(2,), sigma=1e-6, transfer=Sigmoid):
